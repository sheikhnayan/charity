<!DOCTYPE html><!-- Last Published: Thu Aug 28 2025 16:28:18 GMT+0000 (Coordinated Universal Time) --><html data-wf-domain="www.dealmaker.tech" data-wf-page="68547489207784144a773f3e" data-wf-site="656f55af4b70f4ce7ae4b997" lang="en"><head><meta charset="utf-8"/><title>{{ $setting->meta_title ?? 'DealMaker | Raise Capital Online' }}</title><meta content="{{ $setting->meta_description ?? 'DealMaker empowers founders to raise capital online via Reg A, CF, and D. Our tools help companies reach investors and build community from seed to IPO.' }}" name="description"/><meta content="{{ $setting->meta_title ?? 'DealMaker | Raise Capital Online' }}" property="og:title"/><meta content="{{ $setting->meta_description ?? 'DealMaker empowers founders to raise capital online via Reg A, CF, and D. Our tools help companies reach investors and build community from seed to IPO.' }}" property="og:description"/><meta content="{{ $setting->og_image ?? 'https://cdn.prod.website-files.com/656f55af4b70f4ce7ae4b997/685d988c9d3abae4ca097302_opengraphimage.png' }}" property="og:image"/><meta content="{{ $setting->meta_title ?? 'DealMaker | Raise Capital Online' }}" property="twitter:title"/><meta content="{{ $setting->meta_description ?? 'DealMaker empowers founders to raise capital online via Reg A, CF, and D. Our tools help companies reach investors and build community from seed to IPO.' }}" property="twitter:description"/><meta property="og:type" content="website"/><meta content="summary_large_image" name="twitter:card"/><meta content="width=device-width, initial-scale=1" name="viewport"/>@if($setting->meta_keywords)<meta name="keywords" content="{{ $setting->meta_keywords }}"/>@endif<meta content="google-site-verification=cfRfTejLrKY67Lsv3uWZ-Dt1WC9ny_7amMPApbAw-fc" name="google-site-verification"/><link href="{{ asset('css/dealmaker-main.css') }}" rel="stylesheet" type="text/css"/>
...
<header class="n_section_hero dm-main"><div class="n_padding_hero"><div class="container-large is-hero"><div class="n_padding-section-hero"><div class="rl_header44_component"><div class="w-layout-vflex"><div class="rl_header44_spacing-block-2"></div><div class="rl_header44_dmn-flex space margin-bottom margin-medium"><div class="dmn-line max-width-full"></div><div class="text-block-85">{{ $setting->announcement_text ?? 'The Future Of Retail Capital.' }} <a href="{{ $setting->announcement_url ?? '/connect' }}" class="link-4"><span class="text-color-brand-green"><strong>{{ $setting->hero_subtitle ?? 'Raise Boldly' }}</strong></span></a><span class="text-color-brand-green"><strong>.</strong></span></div></div>@if($setting->show_stats ?? true)<div class="w-layout-grid grid-15 on-home-page"><div id="w-node-_7ba4bfb3-22be-9448-20ac-427d4bb0aec6-4a773f3e" class="rl_header44_number-wrapper less-opacity"><div class="counter_number"><div class="n_large-numbers text-color-white">$</div><div fs-numbercount-threshold="0" fs-numbercount-element="number" fs-numbercount-start="1" fs-numbercount-end="{{ $setting->stat_1_number ?? '2' }}" class="n_large-numbers text-color-white">{{ $setting->stat_1_number ?? '2' }}</div><div class="n_large-numbers text-color-white">B+</div></div><div class="rl_header44_dmn-flex space"><div class="dmn-line"></div><div>{{ $setting->stat_1_text ?? 'Raised by customers' }}</div></div></div><div id="w-node-_7ba4bfb3-22be-9448-20ac-427d4bb0aed2-4a773f3e" class="rl_header44_number-wrapper less-opacity"><div class="counter_number"><div class="n_large-numbers text-color-white">{{ substr($setting->stat_2_number ?? '1.5', 0, strpos(($setting->stat_2_number ?? '1.5'), '.') ?: 1) }}.</div><div fs-numbercount-threshold="0" fs-numbercount-element="number" fs-numbercount-start="0" fs-numbercount-end="{{ substr($setting->stat_2_number ?? '1.5', strpos(($setting->stat_2_number ?? '1.5'), '.') + 1) ?: '5' }}" class="n_large-numbers text-color-white">{{ substr($setting->stat_2_number ?? '1.5', strpos(($setting->stat_2_number ?? '1.5'), '.') + 1) ?: '0' }}</div><div class="n_large-numbers text-color-white">M+</div></div><div class="rl_header44_dmn-flex space"><div class="dmn-line"></div><div>{{ $setting->stat_2_text ?? 'Investments processed' }}</div></div></div><div id="w-node-_7ba4bfb3-22be-9448-20ac-427d4bb0aede-4a773f3e" class="rl_header44_number-wrapper less-opacity"><div class="counter_number"><div fs-numbercount-threshold="0" fs-numbercount-element="number" fs-numbercount-start="624" fs-numbercount-end="{{ $setting->stat_3_number ?? '900' }}" class="n_large-numbers text-color-white">{{ $setting->stat_3_number ?? '100' }}</div><div class="n_large-numbers text-color-white">+</div></div><div class="rl_header44_dmn-flex space"><div class="dmn-line"></div><div>{{ $setting->stat_3_text ?? 'Offerings' }}</div></div></div></div>@endif</div></div></div></div></div>
@endif<div id="videoModal" class="video-modal hidden"><div class="html-embed-7 w-embed w-iframe w-script"><style>
#vimeo-mobile {
	display: none;
}
#vimeo-desktop {
  ...
  @media (prefers-reduced-motion: reduce) {
    ...
    <div class="w-slider-mask">
                ...
                <div class="slide w-slide">
                    <div class="w-layout-grid grid-18 is-2-column">
                        <img src="{{ $slide['image'] ?? 'https://cdn.prod.website-files.com/656f55af4b70f4ce7ae4b997/685561045749461ab86204c2_homepage_phone-02.webp' }}" 
                             loading="lazy" width="285.5" sizes="(max-width: 479px) 100vw, 285.5px" alt="{{ $slide['title'] ?? '' }}" 
                             ...
                             class="image-75"/>
                        <div id="w-node-_6baa1412-cc0c-e604-943d-32a5710fea0a-4a773f3e" class="n_platform_texts max-width-small">
                            <img src="https://cdn.prod.website-files.com/656f55af4b70f4ce7ae4b997/685c3250128602cb8c4f0743_dm-p.svg" loading="lazy" alt="" class="platform_logo"/>
                            <div class="spacer-large"></div>
                            <h2 class="n_heading-size-h2 text-color-white max-width-small">{{ $slide['title'] ?? 'Funding Ambition. Powering Growth.' }}</h2>
                            <div class="spacer-small"></div>
                            <p>{{ $slide['description'] ?? 'DealMaker is the future of capital raising. We provide an end-to-end platform to raise capital directly from individual investors. Attract investors, process funds and manage investors on one platform.' }}</p>
                            <div class="spacer-small"></div>
                            <div class="n_button_wrapper">
                                <a href="{{ $slide['cta_url'] ?? '/connect' }}" class="n_button w-button">{{ $slide['cta_text'] ?? 'Start Now' }}</a>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        @else
            ...
            <div class="slide w-slide">
                <div class="w-layout-grid grid-18 is-2-column">
                    <img src="https://cdn.prod.website-files.com/656f55af4b70f4ce7ae4b997/685561045749461ab86204c2_homepage_phone-02.webp" loading="lazy" width="285.5" sizes="(max-width: 479px) 100vw, 285.5px" alt="" srcset="https://cdn.prod.website-files.com/656f55af4b70f4ce7ae4b997/685561045749461ab86204c2_homepage_phone-02-p-500.webp 500w, https://cdn.prod.website-files.com/656f55af4b70f4ce7ae4b997/685561045749461ab86204c2_homepage_phone-02.webp 570w" class="image-75"/>
                    <div id="w-node-_6baa1412-cc0c-e604-943d-32a5710fea0a-4a773f3e" class="n_platform_texts max-width-small">
                        <img src="https://cdn.prod.website-files.com/656f55af4b70f4ce7ae4b997/685c3250128602cb8c4f0743_dm-p.svg" loading="lazy" alt="" class="platform_logo"/>
                        <div class="spacer-large"></div>
                        <h2 class="n_heading-size-h2 text-color-white max-width-small">Funding Ambition. Powering Growth.</h2>
                        <div class="spacer-small"></div>
                        <p>DealMaker is the future of capital raising. We provide an end-to-end platform to raise capital directly from individual investors. Attract investors, process funds and manage investors on one platform.</p>
                        <div class="spacer-small"></div>
                        <div class="n_button_wrapper">
            ...
            <div class="slide w-slide">
                <div class="w-layout-grid grid-18 is-2-column">
                    <img src="https://cdn.prod.website-files.com/656f55af4b70f4ce7ae4b997/6855610466fede381344c563_homepage_phone-03.webp" loading="lazy" width="285.5" sizes="(max-width: 479px) 100vw, 285.5px" alt="" srcset="https://cdn.prod.website-files.com/656f55af4b70f4ce7ae4b997/6855610466fede381344c563_homepage_phone-03-p-500.webp 500w, https://cdn.prod.website-files.com/656f55af4b70f4ce7ae4b997/6855610466fede381344c563_homepage_phone-03.webp 571w" class="image-75"/>
                    <div id="w-node-_8cbac0c2-7a31-ab3d-e798-985b404dc05e-4a773f3e" class="n_platform_texts max-width-small">
                        <img src="https://cdn.prod.website-files.com/656f55af4b70f4ce7ae4b997/685c3250128602cb8c4f0743_dm-p.svg" loading="lazy" alt="" class="platform_logo"/>
                        <div class="spacer-large"></div>
                        <h2 class="n_heading-size-h2 text-color-white"><strong>Raise Boldly. <br/>Own Your Future.</strong></h2>
                        <div class="spacer-small"></div>
                        <p>Unlock the power of retail capital. Raise the capital you need to drive growth while building your brand and community. And unlike venture capital or private equity - you control the terms.</p>
                        <div class="spacer-small"></div>
                        <div class="n_button_wrapper">
                            <a href="/connect" class="n_button w-button">Start Now</a>
                        </div>
            ...
            <div class="slide w-slide">
                <div class="w-layout-grid grid-18 is-2-column">
                    <img src="https://cdn.prod.website-files.com/656f55af4b70f4ce7ae4b997/6855610465b5ca9a46afe153_homepage_phone-04.webp" loading="lazy" width="285.5" sizes="(max-width: 479px) 100vw, 285.5px" alt="" srcset="https://cdn.prod.website-files.com/656f55af4b70f4ce7ae4b997/6855610465b5ca9a46afe153_homepage_phone-04-p-500.webp 500w, https://cdn.prod.website-files.com/656f55af4b70f4ce7ae4b997/6855610465b5ca9a46afe153_homepage_phone-04.webp 571w" class="image-75"/>
                    <div id="w-node-_7633c9cf-c276-6d08-08ca-0d788b3318c0-4a773f3e" class="n_platform_texts max-width-small">
                        <img src="https://cdn.prod.website-files.com/656f55af4b70f4ce7ae4b997/685c3250128602cb8c4f0743_dm-p.svg" loading="lazy" alt="" class="platform_logo"/>
                        <div class="spacer-large"></div>
                        <h2 class="n_heading-size-h2 text-color-white"><strong>Real Capital. <br/>Retail Experience.</strong></h2>
                        <div class="spacer-small"></div>
                        <p>Raise up to $75M annually with Reg A offerings. The capital you need - no road shows, no trips to Sand Hill Road, no waiting for a term sheet. Digital capital raising is changing the game.</p>
                        <div class="spacer-small"></div>
                        <div class="n_button_wrapper">
                            <a href="/connect" class="n_button w-button">Start Now</a>
                        </div>
...
<section class="section_casestudies">
    <div class="w-layout-grid grid-16">
            ...
            @foreach($config->case_studies as $index => $case_study)
                <div id="w-node-_6e3e58cf-7d87-bdef-4976-78bb8065337{{ sprintf('%x', $index + 1) }}-4a773f3e" class="casestudies_boxes {{ $index === 0 ? 'is-full' : 'is-smaller' }}">
                        ...
                        <div>
                            <div class="rl_header44_dmn-flex">
                                ...
                                <div class="rl_header44_number-wrapper">
                                    <div class="rl_header44_dmn-flex space">
                                        ...
                                        <div class="w-layout-hflex">
                                            ...
                                            <div fs-numbercount-threshold="0" fs-numbercount-element="number" fs-numbercount-start="1" fs-numbercount-end="{{ $case_study['capital_raised'] ?? '0' }}" class="n_large-numbers text-color-white">{{ $case_study['capital_raised'] ?? '0' }}</div>
                                    ...
                                    <div class="rl_header44_dmn-flex space">
                                        ...
                                        <div class="w-layout-hflex">
                                            <div fs-numbercount-threshold="0" fs-numbercount-element="number" fs-numbercount-start="1" fs-numbercount-end="{{ $case_study['investors'] ?? '0' }}" class="n_large-numbers text-color-white">{{ $case_study['investors'] ?? '0' }}</div>
                                ...
                                <div class="button-wrapper-new margin-top margin-small">
                                    <a data-w-id="ecd82eb6-55e6-b64f-6fe7-4b8c58107d5{{ $index + 4 }}" href="{{ $case_study['learn_more_url'] ?? '#' }}" class="n_button is-small w-inline-block">
                                        <div>Learn More</div>
                                    </a>
                                </div>
                            </div>
                    ...
                    <div class="casestudies_image">
                        <img src="{{ $case_study['image'] ?? 'https://cdn.prod.website-files.com/656f55af4b70f4ce7ae4b997/685561057298b4672c414ad9_section%203-01.webp' }}" loading="lazy" sizes="(max-width: 852px) 100vw, 852px" alt="{{ $case_study['name'] ?? 'Case Study' }}" class="casestudies_image-inside"/>
        ...
        @else
            ...
            <div id="w-node-_6e3e58cf-7d87-bdef-4976-78bb8065337f-4a773f3e" class="casestudies_boxes is-full">
                <div class="casestudies_text-wrapper bottom-aligned">
                    <div>
                        <div class="rl_header44_dmn-flex">
                            <a href="#" class="casestudies_logo for-rises w-inline-block">
                                <img src="https://cdn.prod.website-files.com/656f55af4b70f4ce7ae4b997/6855710fa0a2d2ed60bd3663_energyx_logo_e7aea357.png" loading="lazy" alt="" class="casestudies_image-size"/>
                            </a>
                            <div class="spacer-medium"></div>
                            ...
                            <div class="rl_header44_number-wrapper">
                                <div class="rl_header44_dmn-flex space">
                                    ...
                                    <div class="w-layout-hflex">
                                        <div class="n_large-numbers text-color-white">$</div>
                                        <div fs-numbercount-threshold="0" fs-numbercount-element="number" fs-numbercount-start="1" fs-numbercount-end="88" class="n_large-numbers text-color-white">31</div>
                                        <div class="n_large-numbers text-color-white">M+</div>
                                    </div>
                                </div>
                                <div class="rl_header44_dmn-flex space">
                                    ...
                                    <div class="n_text-size-tiny">Investors</div>
                                    <div class="w-layout-hflex">
                                        <div fs-numbercount-threshold="0" fs-numbercount-element="number" fs-numbercount-start="1" fs-numbercount-end="31" class="n_large-numbers text-color-white">13</div>
                                        <div class="n_large-numbers text-color-white">K+</div>
                                    </div>
                                </div>
                            </div>
                            <div class="button-wrapper-new margin-top margin-small">
                                <a data-w-id="ecd82eb6-55e6-b64f-6fe7-4b8c58107d54" href="/content/energyx-case-study" class="n_button is-small w-inline-block">
                                    <div>Learn More</div>
                                </a>
                            </div>
                        </div>
                ...
                <div class="casestudies_image">
                    <img src="https://cdn.prod.website-files.com/656f55af4b70f4ce7ae4b997/685561057298b4672c414ad9_section%203-01.webp" loading="lazy" sizes="(max-width: 852px) 100vw, 852px" alt="" class="casestudies_image-inside"/>
                    <div class="n_video_overlay no-radius">
                        <div class="n-play-icon-wrapper"></div>
                    </div>
                </div>
...
@if($config->show_difference_section ?? true)
<div class="spacer-large"></div><header class="n_section_capital"><div class="n_padding-global"><div class="container-large"><div class="padding-section-large-4"><div class="w-layout-grid tabs_content"><div id="w-node-_62e3c04a-cb96-1942-ae5a-f611d7b10a0d-4a773f3e" class="eyebrow text-color-black">{{ $config->difference_eyebrow_text ?? 'DealMaker Difference' }}</div><h2 id="w-node-_9a129288-d4b4-e9d9-03da-7a83b78c9166-4a773f3e" class="n_text_custom max-width-small is-absolut">{{ $config->difference_section_title ?? '#1 in capital raising' }}</h2></div><div data-current="Tab 1" data-easing="ease" data-duration-in="300" data-duration-out="100" class="tabs-3 max-width-large w-tabs"><div class="tabs-menu-2 w-tab-menu"><a data-w-tab="Tab 1" class="tab-link-tab-4 w-inline-block w-tab-link w--current"><div>Plan</div></a><a data-w-tab="Tab 2" class="tab-link-tab-4 w-inline-block w-tab-link"><div>Raise</div></a><a data-w-tab="Tab 3" class="tab-link-tab-4 w-inline-block w-tab-link"><div>Engage</div></a><a data-w-tab="Tab 4" class="tab-link-tab-4 w-inline-block w-tab-link"><div>Repeat</div></a></div><div class="tabs-content-3 w-tab-content"><div data-w-tab="Tab 1" class="w-tab-pane w--tab-active"><div class="w-layout-grid tabs_content is-rev"><div id="w-node-faa7e19f-d2bb-7a65-a229-60b66d5d1c30-4a773f3e" class="code-embed-8 w-embed"><svg width="auto" height="auto" viewBox="0 0 636 500" fill="none" xmlns="http://www.w3.org/2000/svg">
...
<div dark-bg="1" class="footer"><footer dark-bg="1" class="footer_component"><div id="footer" class="padding-global"><div class="container-large"><div class="padding-section-large"><div class="w-layout-grid footer3_top-wrapper"><div class="footer3_left-wrapper"><a href="/old-home-4" class="footer3_logo-link w-nav-brand"><div class="navbar_logo w-embed"><svg width="auto" height="auto" viewBox="0 0 1345 237" fill="none" xmlns="http://www.w3.org/2000/svg">
<g clip-path="url(#clip0_8230_71)">
<path d="M869.37 158.33V235.07H848.21V159.61C848.21 133.81 836.86 120.39 816.99 120.39C795.06 120.39 781.64 136.9 781.64 163.73V235.06H760.48V159.6C760.48 133.8 748.87 120.38 728.75 120.38C707.08 120.38 693.92 138.44 693.92 164.76V235.06H672.76V102.6H691.08L693.92 120.66C700.88 111.12 711.98 101.05 732.36 101.05C750.68 101.05 766.42 109.31 773.9 126.08C781.9 111.89 796.09 101.05 819.57 101.05C846.92 101.05 869.36 116.79 869.36 158.33H869.37Z" fill="white"/>
<path d="M182.43 54.41L121.51 0V105.71L60.78 51.18V181.13L121.76 235.56V180.53L182.43 235.07V54.41Z" fill="white"/>
<path d="M323.75 54.0098H344.94V234.87H326.59L323.75 213.68C314.96 225.82 300.75 236.42 278.53 236.42C242.1 236.42 215.23 211.87 215.23 168.99C215.23 128.68 242.1 101.55 278.53 101.55C300.75 101.55 315.74 110.6 323.75 123.26V54.0098ZM324 169.5C324 140.56 306.43 120.41 280.6 120.41C254.77 120.41 236.93 140.31 236.93 168.99C236.93 197.67 254.5 217.56 280.6 217.56C306.7 217.56 324 197.66 324 169.5Z" fill="white"/>
...
<rect width="1344.03" height="236.42" fill="white"/>
...
</svg></div><div class="spacer-medium"></div></a><div class="spacer-small"><div class="text-size-small">DealMaker provides comprehensive capital raising technology that transforms how companies raise funds, engage investors, and build community.<br/><br/>30 East 23rd St. Fl. 2 <br/>New York, NY 10010</div><div class="spacer-medium"></div><div class="w-layout-grid footer3_social-list">
                                @if($config->linkedin_url)
                                <a aria-label="LinkedIn (opens in a new tab)" href="{{ $config->linkedin_url }}" target="_blank" class="footer3_social-link w-inline-block">
                                ...
                                <a aria-label="Twitter/X (opens in a new tab)" href="{{ $config->twitter_url }}" target="_blank" class="footer3_social-link w-inline-block">
                                    <div class="icon-embed-xxsmall is-small footer-icon w-embed">
                                        <svg width="auto" height="auto" viewBox="0 0 336 328" fill="none" xmlns="http://www.w3.org/2000/svg">
                                ...
                                <a aria-label="Facebook (opens in a new tab)" href="{{ $config->facebook_url }}" target="_blank" class="footer3_social-link w-inline-block">
                                    <div class="icon-embed-xxsmall footer-icon w-embed">
                                        <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" aria-hidden="true" role="img" class="iconify iconify--bx" width="100%" height="100%" preserveAspectRatio="xMidYMid meet" viewBox="0 0 24 24">
                                ...
                                <a aria-label="Instagram (opens in a new tab)" href="{{ $config->instagram_url }}" target="_blank" class="footer3_social-link w-inline-block">
                                    <div class="icon-embed-xxsmall footer-icon w-embed">
                                        <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" aria-hidden="true" role="img" class="iconify iconify--bx" width="100%" height="100%" preserveAspectRatio="xMidYMid meet" viewBox="0 0 24 24">
                                ...
                                @endif
                            </div></div><div class="spacer-medium"></div><img width="205" loading="lazy" alt="" src="https://cdn.prod.website-files.com/656f55af4b70f4ce7ae4b997/65784614f244b62e543d68de_Deloitte%20Companies%20to%20watch%20award%20(Facebook%20Cover)%20(4)%201.png"/></div><div class="footer3_right-wrapper"><div class="w-layout-grid footer3_menu-wrapper">
                            @if($config->footer_menu_raise_capital && count($config->footer_menu_raise_capital) > 0)
                            <div class="footer3_link-list">
                                <div class="text-size-medium capitalize text-color-gray">RAISE CAPITAL</div>
                                <div class="line-sepearation is-small"></div>
                                <div class="spacer-small"></div>
                                @foreach($config->footer_menu_raise_capital as $link)
                                    <a href="{{ $link['url'] ?? '#' }}" class="footer3_link">{{ $link['title'] ?? 'Link' }}</a>
                                @endforeach
                            </div>
                            @endif
                            
                            @if($config->footer_menu_solutions && count($config->footer_menu_solutions) > 0)
                            <div class="footer3_link-list">
                                <div class="text-size-medium capitalize text-color-gray">OUR SOLUTIONS</div>
                                <div class="line-sepearation is-small"></div>
                                <div class="spacer-small"></div>
                                @foreach($config->footer_menu_solutions as $link)
                                    <a href="{{ $link['url'] ?? '#' }}" class="footer3_link">{{ $link['title'] ?? 'Link' }}</a>
                                @endforeach
                            </div>
                            @endif
                            
                            @if($config->footer_menu_company && count($config->footer_menu_company) > 0)
                            <div class="footer3_link-list">
                                <div class="text-size-medium capitalize text-color-gray">COMPANY</div>
                                <div class="line-sepearation is-small"></div>
                                <div class="spacer-small"></div>
                                @foreach($config->footer_menu_company as $link)
                                    <a href="{{ $link['url'] ?? '#' }}" class="footer3_link">{{ $link['title'] ?? 'Link' }}</a>
                                @endforeach
                            </div>
                            @endif
                            
                            @if($config->footer_menu_resources && count($config->footer_menu_resources) > 0)
                            <div class="footer3_link-list">
                                <div class="text-size-medium capitalize text-color-gray">RESOURCES</div>
                                <div class="line-sepearation is-small"></div>
                                <div class="spacer-small"></div>
                                @foreach($config->footer_menu_resources as $link)
                                    <a href="{{ $link['url'] ?? '#' }}" class="footer3_link">{{ $link['title'] ?? 'Link' }}</a>
                                @endforeach
                            </div>
                            @endif
                            ...
                            <div class="footer3_link-list">
                                <div class="text-size-medium capitalize text-color-gray">STAY UPDATED</div>
                                <div class="line-sepearation is-small"></div>
                                <div class="spacer-small"></div>
                                <div class="text-size-small">{{ $config->footer_newsletter_description ?? 'Subscribe to our newsletter for the latest updates and insights on capital raising.' }}</div>
                                <div class="spacer-medium"></div>
                                <div class="w-embed w-script"><script charset="utf-8" type="text/javascript" src="//js.hsforms.net/forms/embed/v2.js"></script>
<script>
  hbspt.forms.create({
    portalId: "7493765",
